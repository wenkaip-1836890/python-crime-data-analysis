...
class Question2:
    ...
    def _report_predict(self, crime_type, community, time):
        """
        a function that is only used to automaticlly get the result
        in the report
        """
        print('Please enter a community area number: ' + str(community))
        print('Please enter a crime type: ' + str(crime_type))
        print('Please enter the hour: ' + str(time))
        crime_type = crime_type.upper()
        community = float(community)
        time = float(time)
        keys = (self._X_test.columns)
