...
def merge_data(geo_data, population_data):
    ...
    population_data['CENSUS_BLOCK_11_DIGIT'] =\
        population_data['CENSUS BLOCK FULL'] // 10000
    population_data['CENSUS_BLOCK_11_DIGIT'] =\
        population_data['CENSUS_BLOCK_11_DIGIT'].astype(str)
...
def generate_crime_arrest_rate(crime_data, geo_pop_data):
    ...
    pop_data_per_ca =\
        geo_pop_data.groupby('commarea_n')['TOTAL POPULATION'].sum()
    ...
    case_pop_data['crime_rate'] =\
        case_pop_data['case_num'] / case_pop_data['TOTAL POPULATION']
...
def Question3(crime_data, geo_data, population_data):
    ...
    percent = corr_ca[corr_ca['correlation'].abs() > 0.5]['correlation'].\
        count() / len(corr_ca)
