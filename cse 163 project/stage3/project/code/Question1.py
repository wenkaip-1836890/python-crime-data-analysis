...
class Question1:
    ...
    def plot_communities_single_year(self):
        ...
        year should be between 2002-2018
        """
        plot_again = 'True'
    ...
    def plot_change_through_years(self):
        ...
        community numbers are in 1-77
        """
        communities_str = input('Enter communities you want to plot: ')
        self._plot_change(communities_str)

    def _plot_change(self, communities_str):
        """
        a function that is only used to automaticlly get the result
        in the report
        """
        ...
        plt.savefig('change_through_years.png')

    def _plot_all_Ca(self):
        sns.relplot(data=self._total_frame, kind="line", x="Year",
                    y="Sentence Length in Month", hue='Community Area')
        plt.savefig('change_through_years_all_area.png')
    ...
    def safety_ranking(self):
        ...
        year = int(year)
        self._safety_ranking(year)

    def _safety_ranking(self, year):
        """
        a function that is only used to automaticlly get the result
        in the report
        """
